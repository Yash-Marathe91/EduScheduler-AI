...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // If Supabase fails, we proceed without a user (will trigger login redirect if not public route)
  }
  ...
  if (!isApiRoute) {
    ...
    if (user && isAuthRoute) {
      // user is logged in, redirect to dashboard if they try to access auth routes
      const url = request.nextUrl.clone()
